$(window).on('load',function(){$('.preloader').fadeOut(100);});function handle_click_zoom(el){$(el).find(".portfolio-popup").click();}
var myShuffle="";$(document).ready(function(){'use strict';var containerEl=document.querySelector('.shuffle-wrapper');if(containerEl){var Shuffle=window.Shuffle;myShuffle=new Shuffle(document.querySelector('.shuffle-wrapper'),{itemSelector:'.shuffle-item',buffer:1,group:"automation"});jQuery('input[name="shuffle-filter"]').on('change',function(evt){var input=evt.currentTarget;if(input.checked){myShuffle.filter(input.value);}});}
